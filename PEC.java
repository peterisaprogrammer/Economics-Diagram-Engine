...
public class PEC extends SandDdiagram {
    private static int MSB_Y_Intercept;

    public PEC(int SupplyGradient, int Demand_Y_Intercept, int DemandGradient, int MSB_Y_Intercept){
        super(SupplyGradient, Demand_Y_Intercept, DemandGradient);
        PEC.MSB_Y_Intercept = MSB_Y_Intercept;
    }
    ...
    public JFreeChart createChart(){
        ...
        for (int p = 0; p <= 50; p++) {
            demandMSB.add(p, MSB_Y_Intercept - DemandGradient * p);
        }

        XYSeries demandMPB = new XYSeries("D=MPB");
        for (int p = 0; p <= 50; p++) {
            demandMPB.add(p, Demand_Y_Intercept - DemandGradient * p);
        }
        ...
        double eqPriceOPT = (double) MSB_Y_Intercept/(SupplyGradient+DemandGradient);
